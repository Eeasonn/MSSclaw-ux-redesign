...
import { SkillReviewQueue } from '@/components/center/SkillReviewQueue';
import { SkillUploadModal } from '@/components/center/SkillUploadModal';
...
export function SkillCenterPage({ onInvoke }: SkillCenterPageProps) {
  ...
  // 未过审技能仅上传者本人可见；运营通过「待审核」队列处理
  const list = filteredSkills().filter((s) =>
    isSkillVisibleToUser(s, getCurrentUserId(), getCurrentUserName()),
  );
      ...
      <div className="mx-auto max-w-6xl">
        <CenterPageHeader
          ...
          tip={
            isBusiness ? (
              <>
                浏览团队已上线的技能，输入 <code className="rounded bg-black/[0.04] px-1">/skill名</code>{' '}
                在任务中心调用；也可以上传自己的技能，审核通过后对全团队上线。
              </>
            ) : (
              <>
                Skill 可按 NP 与区域归属上架。下载为 Skill 包（含 SKILL.md、reference、templates、assets）；也可导入
                ZIP / SKILL.md / JSON。输入 <code className="rounded bg-black/[0.04] px-1">/skill名</code>{' '}
                在任务中心调用。
              </>
            )
          }
          ...
          {list.length ? (
            list.map((s) => (
              <div key={s.id} className="market-card apple-card flex flex-col p-4">
                ...
                {getSkillReviewStatus(s) === 'rejected' && s.rejectReason && (
                  <p className="mt-1.5 rounded-lg border border-red-100 bg-red-50/60 px-2 py-1.5 text-[10px] leading-relaxed text-red-600">
                    驳回原因：{s.rejectReason}
                  </p>
                )}
