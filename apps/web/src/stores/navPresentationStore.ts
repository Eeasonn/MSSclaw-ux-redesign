...
function enableBusinessSkills(roleEnabled: RoleNavMatrix): RoleNavMatrix {
  return {
    ...roleEnabled,
    business_user: { ...roleEnabled.business_user, skills: true },
    viewer: { ...roleEnabled.viewer, skills: true },
  };
}

/** v5 → v6：保留既有勾选，强制为业务/只读角色补开协作室与找专家入口 */
function enableBusinessCollabAndExperts(roleEnabled: RoleNavMatrix): RoleNavMatrix {
  return {
    ...roleEnabled,
    business_user: { ...roleEnabled.business_user, warroom: true, agents: true },
    viewer: { ...roleEnabled.viewer, warroom: true, agents: true },
  };
}
