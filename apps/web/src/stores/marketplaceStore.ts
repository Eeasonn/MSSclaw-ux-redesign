...
interface MarketplaceState {
  ...
  /** 运营审核：通过则正式上线，驳回则记录原因 */
  reviewSkill: (id: string, approved: boolean, reason?: string) => void;
  /** 运营审核队列：待审核技能 */
  getPendingReviewSkills: () => PrototypeSkillSeed[];
...
export const useMarketplaceStore = create<MarketplaceState>((set, get) => ({
  ...
  uploadSkill: ({ name, desc, category, instructions }) => {
    const userName = getCurrentUserName() || '业务用户';
    ...
    get().showToast(`技能「${name}」已提交，等待运营审核`);
  },

  reviewSkill: (id, approved, reason) => {
    const target = get().skills.find((s) => s.id === id);
    if (!target) return;
    get().upsertSkill(
      approved
        ? { ...target, reviewStatus: 'approved', published: true, rejectReason: undefined }
        : {
            ...target,
            reviewStatus: 'rejected',
            published: false,
            rejectReason: reason?.trim() || '内容不符合平台上架规范，请修改后重新提交。',
          },
    );
    get().showToast(
      approved ? `技能「${target.name}」已通过审核并上线` : `技能「${target.name}」已驳回`,
    );
  },

  getPendingReviewSkills: () =>
    get().skills.filter((s) => (s.reviewStatus ?? 'approved') === 'pending'),
