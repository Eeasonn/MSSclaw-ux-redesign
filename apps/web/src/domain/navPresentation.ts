...
/**
 * 可配置的侧栏槽位：含 AppView + 业务壳专属「协作室」。
 * 协作室不是独立路由页，而是任务区下的会话分组入口。
 */
...
export const NAV_PRESENTATION_META: NavPresentationMeta[] = [
  ...
  { id: 'new-task', label: '开启任务', subtitle: 'AI 任务发起页', icon: 'fa-sparkles', section: 'workspace', hiddenFromSidebar: true },
  { id: 'warroom', label: '协作室', subtitle: '群组会话 · 多专家协作（业务侧栏一级入口）', icon: 'fa-comments', section: 'workspace' },
  { id: 'messages', label: '我的消息', subtitle: '推送通知 · 顶栏铃铛入口', icon: 'fa-bell', section: 'workspace', hiddenFromSidebar: true },
  { id: 'ai-campus', label: 'AI学院', subtitle: '系统学 AI，把能力装进工作流', icon: 'fa-graduation-cap', section: 'workspace' },
  { id: 'ai-campus-all', label: '全部课程', subtitle: 'AI学院课程列表', icon: 'fa-graduation-cap', section: 'workspace', hiddenFromSidebar: true },
  ...
  { id: 'agent-studio', label: 'Agent Studio', subtitle: '已并入专家页配置', icon: 'fa-wand-magic-sparkles', section: 'platform', hiddenFromSidebar: true },
  ...
  { id: 'cases', label: '案例库', subtitle: '已并入案例样板间', icon: 'fa-lightbulb', section: 'platform', hiddenFromSidebar: true },
  ...
  { id: 'portal-ops', label: '门户运营', subtitle: '前沿洞察与培训赋能上架', icon: 'fa-newspaper', section: 'system', locked: true, adminOnly: true },
  { id: 'admin', label: '组织权限', subtitle: '组织 · 部门 · 角色 · 成员治理', icon: 'fa-shield-halved', section: 'system', adminOnly: true },
  { id: PRESENTATION_CONFIG_VIEW, label: '展示配置', subtitle: '按角色配置侧栏菜单', icon: 'fa-sliders', section: 'system', locked: true, adminOnly: true },
  { id: WORKSPACE_CONFIG_VIEW, label: '租户配置', subtitle: '数据空间 · 租户（仅超级管理员）', icon: 'fa-building', section: 'system', locked: true, adminOnly: true },
];
...
export const NAV_PRESET_LABELS: Record<NavPresetId, { title: string; description: string }> = {
  customer: {
    title: 'MVP演示',
    description: '按角色裁剪：业务侧开放协作室与找专家；运营侧保留专家/技能；治理仅超管',
  },
  standard: {
    title: '标准能力',
...
/** MVP：业务/访客开放协作室与找专家（只读浏览）；运营有任务+协作室+专家技能 */
function mvpForRole(role: PlatformRole): Record<NavSlotId, boolean> {
  ...
  if (role === 'viewer') {
    // 只读：可看广场发现/案例/技能/专家/协作室与已有任务结果；不强调执行入口
    return withAdminLocks(
      {
        ...off,
        home: true,
